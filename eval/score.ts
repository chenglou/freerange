...
//     [--slice <name>]... [--unit <id>]... [--timeout-seconds 300]
//     [--node-modules <label>=<dir>] [--examples-per-site 3] [--max-runtime-runs 3000]
...
async function main(): Promise<void> {
  ...
  for (const unit of selected) {
    ...
    if (checkable.length > 0 && !options.skipRuntime) {
      const selection = selectExamples(checkable.map(({site}) => ({key: site.key, available: truthByKey.get(site.key)!.examples.length})), options.examplesPerSite, Math.max(0, options.maxRuntimeRuns - runtimeRuns))
