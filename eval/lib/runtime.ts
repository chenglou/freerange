...
export function selectExamples(sites: Array<{key: string; available: number}>, perSite: number, total: number): {selected: ExampleCandidate[]; skipped: number} {
  const selected: ExampleCandidate[] = []
  let skipped = 0
  for (const site of sites) {
    const wanted = Math.min(site.available, perSite)
