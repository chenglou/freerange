...
export function applyInsertion(treeRoot: string, path: string, pristineSha1: string, withInsertion: string): {ok: boolean; detail: string; diff: string} {
  const filePath = join(treeRoot, path)
  const current = readFileSync(filePath, 'utf8')
  if (sha1(current) === sha1(withInsertion)) return {ok: true, detail: 'already inserted', diff: git(treeRoot, ['diff', '--no-color', '--', path]).stdout}
  if (sha1(current) !== pristineSha1) return {ok: false, detail: `${filePath} holds neither the commit's file nor the insertion`, diff: ''}
  writeFileSync(filePath, withInsertion)
