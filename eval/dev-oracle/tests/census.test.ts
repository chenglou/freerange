...
const names = {imports: new Set(['layoutLines']), declared: new Set(['layoutLines', 'canvasContext', 'clamp', 'data'])}
...
test('groups a function\'s not-lowered finding by its first unsupported construct', () => {
  ...
  expect(classifyMessage(notChecked('function call layoutLines'), names).group).toBe('import')
