...
- grid row metadata: `bottom == top + height`, nondecreasing row tops, and row gaps
...
- `getGridLayout` now proves the row-height packing loop's row metadata and gaps. It still does not prove the row count relation or item-to-row/column adjacency.
