...
First cleanup pass after cutover: contract collection, function input binding, `given` validation/assumption seeding, domain-path value access, helper-call precondition/summary logic, and `shape-diff` comparison moved out of `src/check-core.ts`. The important collapse was `shape-diff`: implication checks now ask typed range/equality facts from interpreter values instead of reparsing printed fact text. Keep extracting only where the boundary makes the code simpler or moves a recognizer into the fact layer; the public language does not need to grow for that.
