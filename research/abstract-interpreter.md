...
The useful comparison was behavioral, not architectural. The old evaluator helped reveal missing surfaces, then got deleted with the differential harness once demos, negative reports, inferred facts, focused interpreter tests, and type/lint all had enough coverage.
...
First cleanup pass after cutover: contract collection, function input binding, `given` validation/assumption seeding, domain-path value access, and helper-call precondition/summary logic moved out of `src/check-core.ts`. The important collapse was the old shape comparison: implication checks now ask typed range/equality facts from interpreter values instead of reparsing printed fact text. Keep extracting only where the boundary makes the code simpler or moves a recognizer into the fact layer; the public language does not need to grow for that.
