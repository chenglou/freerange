...
The first consolidation pass after the indexed-loop sprint made scalar reducer capture explicit: fresh `for..of` and indexed loops now share one pending reducer object, one reducer recognizer, and one reducer finalizer. Push/origin handling is still split, but the loop bodies no longer each spell out conditional-add, scalar-add, and extremum bookkeeping.
