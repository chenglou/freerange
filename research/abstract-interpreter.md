...
First cleanup slice after cutover: `shape-diff` comparison moved out of `src/check-core.ts`, and its implication checks now ask typed range/equality facts from interpreter values instead of reparsing printed fact text. Keep extracting clear pieces from `src/check-core.ts` where the move also collapses a recognizer into the fact layer; the public language does not need to grow for that.
