...
Guarded scalar totals are the separate small reducer shape: `if (item.visible) count += 1` or `if (item.visible) total += item.height` produces the conditional running-sum range and the extra comparison facts such as `count <= items.length` when the source proves them. Scalar extrema are another pure reducer shape: `maxWidth = Math.max(maxWidth, item.width)` and `minWidth = Math.min(minWidth, item.width)` keep the target range bounded by the start value and the item domain. For now, these reducer loops cannot also push rows, mix in cursor updates, or use an `else` branch. Those combinations stay on the legacy path until the fresh loop state can model them directly.

The fresh core also has the first indexed `for` loop shape: `for (let i = 0; i < limit; i++) values.push(i)`. The limit must already be a non-negative integer. The pushed index gets the same element-path range and comparison assumptions as the old indexed loop path, so `values[]: int 0..<limit` proves from source instead of from a special public atom. Array-source indexed loops and row/cursor indexed loops are still legacy-only.
