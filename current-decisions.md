...
- Type assertions written with `as` or angle brackets. An assertion changes the static type without changing the value, e.g. `true as unknown as number` puts a boolean where every downstream computation expects a number. Rejected wholesale. The non-null assertion `x!` stays accepted where it does not change the value kind; when array support lands, `array[index]!` under noUncheckedIndexedAccess is everywhere in real code and needs its own explicit treatment — an assumption line or a bounds proof, not a loophole.
