// astmut@v1 (plan-a/registered/m7-mj-gallery.json `operators`): the operator mutants of one in-scope mj-gallery file, generated
// from the file's TypeScript AST. astmut.ts runs it on each pinned file to write m7's table; contract-writing.ts runs it on a
// writer's patched file to map m7's rows onto that file. It never reads an assert expression: console.assert calls only mark the
// lines no mutant may touch, and pick which functions are in the mutation scope. No function under test runs.
...
import type {AstmutCopy, AstmutOperator, AstmutRow} from './mj-gallery.ts'
...
/**
 * Every astmut@v1 row of one file, e.g. m7-c02-midui-001 to m7-c02-midui-014 for src/MidUI/MidUI.ts, and the copy's counts.
 * `path` is where `text` sits on disk, in a tree whose tsconfig.json resolves the file's imports; `file` is the path the rows and
 * change rules name, e.g. `src/MidUI/MidUI.ts`.
 */
export function astmutFile(input: {copy: string; file: string; path: string; text: string; exportShim: string[]; excluded: string[]}): {rows: AstmutRow[]; copy: AstmutCopy} {
  const {copy, file, path, text} = input
  const logicalName = basename(file, extname(file))
  const program = loadProgram([path])
  const sourceFile = program.getSourceFile(path)
  if (sourceFile == null) throw new Error(`TypeScript did not load ${path}`)
  if (sourceFile.text !== text) throw new Error(`${path}: the text on disk differs from the text given`)
  const checker = program.getTypeChecker()
  const blocked = assertLines(sourceFile)
  const scope = mutationScope(checker, sourceFile, input.exportShim, input.excluded)
  ...
  if (ts.transpileModule(text, {reportDiagnostics: true, fileName: path}).diagnostics?.length !== 0) throw new Error(`${file}: the file has syntax diagnostics`)
  ...
  candidates.forEach((candidate, index) => {
    const id = `m7-${copy}-${String(index + 1).padStart(3, '0')}`
    ...
    const change = {file, from: text.slice(span.from, span.to), to: text.slice(span.from, candidate.start) + candidate.after + text.slice(candidate.end, span.to)}
    ...
    rows.push({
      id, copy, file, function: candidate.functionName, line: position.line + 1, column: position.character + 1, operator: candidate.operator,
  ...
  return {
    rows,
    copy: {
      copy, file, scopeFunctions: scope.filter((declaration) => declaration.kind === 'function').map((declaration) => declaration.name),
