...
- Symbolic index precision should use the same small case layer. For finite arrays and tuples, exact index cases like `0 | 2` can select only those slots; broad index ranges should keep the old interval join instead of pretending every symbolic index is understood.
- Nullable shape support should stay tied to real TypeScript control flow for now. A guarded read of an optional property can use the narrowed shape at that expression; an unguarded optional path should remain unknown rather than becoming an optimistic nullable object domain.
