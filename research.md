...
- The aliasing fix belongs at helper binding, not in each proof rule. Caller numeric values now carry their bottomed-out linear form through local aliases and callee parameters, so `max = cols - w` can prove a helper's `max >= min` from the original `given w <= params.cols`. The same pass exposed that fixed tuple slots like `extent[1][0]` are honest input paths for `given` lines; dynamic `items[index]` is still source proof, not an input fact.
