...
- `fr check --audit` should stay source-selector-shaped, not become a style linter. The current family is `Math.min`, `Math.max`, and exact min/max ternaries where one guard value or branch cannot affect the result under current facts. Userland clamp helpers and assignment-style clamps should join only when they report through the same pure-choice shape instead of one helper-specific warning.
