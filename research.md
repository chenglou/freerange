...
- `items.map(...)` preserves length, simple field domains, optional callback index facts, and a same-index origin fact from the collection summary, not by unrolling every literal slot. `items.filter(...)` records an order-preserving subset origin, and map/filter chains now compose that origin back to the base source for `fr infer`. Expression callbacks and tiny block callbacks with consts/return branches are enough for the code we have seen; richer source-order facts can come later when there is a public fact that needs them.
