...
Collection/sequence/proof-context/numeric ownership should keep leaving the general value domain. `src/array-summary.ts` now owns summary merging and empty-branch lineage joins; `src/assumptions.ts` owns linear/comparison assumption identity; `src/number-domain.ts` owns numeric construction, finite cases, arithmetic, running sums, and extrema; `src/domain.ts` still re-exports those helpers while callers settle. That is the model for future domain splitting: move one coherent ownership boundary, keep behavior flat, then let imports migrate when the payoff is clear.
