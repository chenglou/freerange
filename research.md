...
Function and variable identities come from TypeScript bindings, not identifier text. Keep symbols while tracking reads, writes, and aliases. One callable resolver follows function declarations, `const` function bindings, immutable aliases, imports with TypeScript source, inline callbacks, and synchronous IIFEs. A resolved function carries the TypeScript program that contains it; effect and purity analysis use the function and program together as one reference. Calls through mutable bindings, object or array properties, conditionals, or another call's result are unknown. Keep import provenance separate because it describes the import path used by the caller. Convert identities back to source names only for reports and interpreter root invalidation.
...
Describe a free function's effects once: outside reads, argument mutation, retained arguments, callback effects, environment observation, throwing, calls whose behavior is unavailable, and whether the returned outer object is new or comes from an argument. Direct calls, imports, callbacks, contract helper calls, expression rechecking, and loop bounds all use that description. Do not carry references inside returned objects or arrays across the call boundary. For example, mutating `copyArray(rows)` itself is local, while mutating `copyArray(rows)[0]` is unknown.

Platform calls use one table with the same information. Callback entries name the callback argument, what each callback parameter can reach, and any written `thisArg` needed for conservative interpreter invalidation. Mutation and retention entries name the receiver or exact argument positions. Platform return entries describe only the outer result as new, the receiver, an argument, absent, or unknown. The interpreter may know more about a supported result, such as the length of `items.map(...)`, but it still gets call effects from this table. This model assumes ordinary built-in behavior; it does not account for proxies, a subclass widened to a built-in base type, or code that replaces a built-in method.

Report behavior as unknown instead of adding another partial recognizer. Any use of `this`, recursion, async functions, generators, `try`/`catch`, class member calls, and user construction are unknown. Class calls remain unknown even when source is visible because runtime dispatch, base constructors, and field initializers involve more than one selected body. Platform operations with hidden iteration, property reads, conversion, or default ordering stay unknown until the operation has a complete effect rule. Mutable module objects and arrays are outside state; only immutable primitive bindings are stable reads.
