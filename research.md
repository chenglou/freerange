...
- Nullable shape support should stay tied to real TypeScript control flow for now. A guarded read of an optional property can use the narrowed shape at that expression; optional numeric values can feed normal math after `typeof value !== 'undefined'`; a nullable local or property path constructed from branch source facts can expose its present-side facts after `== null` / `!= null`; an unguarded optional path should remain unknown rather than becoming an optimistic nullable object domain.
