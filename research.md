...
- `items.map(...)` preserves length, simple field domains, optional callback index facts, and a same-index origin fact. `items.filter(...)` records an order-preserving subset origin, and map/filter chains now compose that origin back to the base source for `fr infer`. Expression callbacks and tiny block callbacks with consts/return branches are enough for the code we have seen; richer source-order facts can come later when there is a public fact that needs them.
- `items.filter(...)` is worth supporting as a boring subsequence summary: output items keep the source item domain, output length is at most source length, and `infer` can say the output is an order-preserving subset. Do not let that grow into predicate logic yet.
...
- Symbolic index precision should use the same small case layer. For finite arrays and tuples, exact index cases like `0 | 2` can select only those slots; broad index ranges should keep the old interval join instead of pretending every symbolic index is understood. Specific source reads like `items[focused].top` now keep concrete path terms, and local adjacent sequence facts can instantiate previous/current and current/next neighborhoods once bounds prove them live.
...
- Else-if support was a control-flow hole, not a new proof idea. TypeScript represents `else if` as an `IfStatement` inside the outer `else`, so the right fix was to evaluate that nested branch with the same continuation. That made xyflow's `calcAutoPanVelocity` ordinary source again and moved the focused `general`/resizer slice to 27 checked claims and 15 clean doctor calls.
- The first `fr scout --function name` pass is useful but not satisfying enough to promote inferred contracts. It speculates over simple return-vs-param comparisons and scans calls against the resulting provisional requirements. That exposes real-looking obligations like `min <= max`, but also inverse facts like `max <= min` when the speculative candidate was silly. Keep it read-only and scoped; the durable path is still explicit contracts plus `infer`, not generated law.
...
- The second external sweep added `floating-ui`, `dnd-kit`, `react-grid-layout`, `d3-dag`, `Chart.js`, and `fabric.js`. The useful new pressure is not another one-off atom:
  ...
  - `dnd-kit` and `fabric.js` put the cleanest geometry behind class methods and getters: rectangle `right`, `bottom`, `center`, `area`, `containsPoint`, intersection area, rounded-rect radii. Freerange now supports local `@fit` checks on class methods/getters with `this` as an input root, plus same-file member summaries for ordinary property/method calls. Imported class-member summaries are still out of scope.
...
The resolver follows named imports through TypeScript module resolution when they land on local source files, and it can recover local source from a declaration file with one source in its declaration map. It also follows explicit named re-export barrels and top-level `const` helper bindings when each hop is statically known and the final target is a supported `Math` call, same-file helper, or local-source imported helper. That is enough for relative helpers, `.tsx` helpers, `tsconfig` path aliases, built workspace packages with source maps, and code like `export const max = Math.max` without committing to arbitrary package exports, declaration-only summaries, wildcard barrels, stale-summary trust, mutable helper bindings, or workspace caches.
