...
- Disjunction should live at the right level. The current numeric `cases` layer is fine for one scalar's small finite alternatives, but branch/path alternatives really belong on whole interpreter states. Otherwise correlated branch facts can split apart: `if (b) { x = 1; y = 10 } else { x = 2; y = 20 }` becomes `x: 1 | 2` and `y: 10 | 20`, and later `x + y` can invent impossible combinations. The cleaner rule is: keep control-flow either/or as state-level cases, then compress to value-level cases only when all other tracked facts are identical. The first implementation keeps a small state budget; over budget, it preserves identical facts, forgets facts that vary by branch, and records a budget message. If that becomes too conservative, forget whole split dimensions deterministically and call that partition merging, not loop widening.
