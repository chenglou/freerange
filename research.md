...
- Whole-value specs cover the contract side of that boundary. `return: {left: 0, width: 100} | {left: 20, width: 80}` checks paired object cases from return cases; `return.left: 0 | 20` only checks the flattened field. Keep this distinction visible in docs and reports because it is the user's way to choose between a cheap field fact and a paired value fact.
