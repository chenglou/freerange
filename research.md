...
- `items.map(...)` preserves length, simple field domains, optional callback index facts, and an immediate same-index origin fact. Expression callbacks and tiny block callbacks with consts/return branches are enough for the code we have seen; source order can come later when there is a public fact that needs it.
...
The resolver follows named imports through TypeScript module resolution when they land on local source files, and it follows explicit named re-export barrels. It also follows top-level `const` helper bindings when each hop is statically known and the final target is a supported `Math` call, same-file helper, or local-source imported helper. That is enough for relative helpers, `.tsx` helpers, `tsconfig` path aliases, and code like `export const max = Math.max` without committing to package exports, declaration-only imports, wildcard barrels, summary files, stale-summary trust, mutable helper bindings, or workspace caches.
