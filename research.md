...
- The second external sweep added `floating-ui`, `dnd-kit`, `react-grid-layout`, `d3-dag`, `Chart.js`, and `fabric.js`. The useful new pressure is not another one-off atom:
  ...
  - `dnd-kit` and `fabric.js` put the cleanest geometry behind class methods and getters: rectangle `right`, `bottom`, `center`, `area`, `containsPoint`, intersection area, rounded-rect radii. Freerange now supports local `@fit` checks on class methods/getters with `this` as an input root, which unlocks these source seams without moving helpers out of classes. The next question is whether class-member summaries at property/method call sites are worth the extra machinery.
