...
- The April corpus report sweep across `xyflow`, `tldraw`, `fabric.js`, `interact.js`, `2d-geometry`, `dagre`, `d3-scale`, `gridstack.js`, `angular-grid-layout`, and `moveable` mostly found adoption polish, not proof-language pressure. Stale `result` comments needed an explicit "`return`, not `result`" hint. Tuple-heavy files needed readable fixed array lengths like `return.length == 5`, not recurrence noise. Unknown identifiers should propagate through `result[3]` and `result.left + result.right` so the report names the real blocker instead of saying "expected an array" or "expected numbers."
