...
Empirically, this prompt template has proven very effective at finding various correctness issues when we need to achieve a goal. The subagents with differing perspectives help a lot. An independent review of the finished change by a second model also helped. The template no longer asks for that review, because it was too expensive.
...
Before editing, turn every explicit requirement into one small concrete example with its exact observable result. Trace the relevant information through every place that creates, carries, combines, invalidates, or consumes it, and test each place where passing the information from one part of the code to another can change the result; a nearby example or a test that repeats the implementation's assumption does not count. Pair every required rejection or less precise result (`unsupported`, `partially supported`, `assertion unproven`, an `assumes:` line in place of a `requires:` line, or a wider range) with a positive control through the same path.
...
When two requirements use the same representation or consumer, test them together. When an operation is supported, repeat it at least twice to verify that the operation still works when repeated or combined.
...
Use fresh subagents before editing to independently explore important scope, representation, and verification risks. Then implement the smallest coherent behavior required by the task, with focused positive and negative tests. Prefer a clear rejection (`unsupported`) or a less precise result over ad-hoc support. Narrow, pivot, or revert if implementation shows that the task is worse than expected.
