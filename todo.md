...
0. **Keep the interpreter as the one source evaluator.**
   Function bodies, top-level inline checks, local inline checks, return checks, object-field checks, type-boundary checks, loop reports, and helper-call obligations now run through the interpreter. The old evaluator and differential harness are gone. `src/check.ts` is now the public front door; keep extracting clear pieces from `src/check-core.ts` and keep moving report wording toward the same fact inventory.
