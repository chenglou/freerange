...
- **Direct call mutating a contained element path** is the residual element-alias case (a narrow false-PROVE). `const box = {v: 1}; const arr = [box]; grow(arr[0])` mutates box through `arr[0]`, but the call-effect havoc forgets only the container root `arr`, not its contained bindings. The frame's containment map (`arr` holds `box`, from a `[box]` literal, a push, or `const arr2 = arr`) is consulted when an array-method *callback* mutates elements, so `forEach`/`map`/`Array.from` are covered; extend it to direct calls whose mutated argument is an element/field access into a tracked container. The reverse order, `const first = arr[0]` then mutating `first`, is already sound (distinct Value identity).
