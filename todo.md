...
- Numeric-limit indexed loops like `for (let i = 0; i < limit; i++) values.push(i)` preserve `values.length == limit` and `values[]: int 0..<limit`. Array-source indexed loops like `for (let i = 0; i < items.length; i++) rows.push({sourceIndex: i})` can also use `items[i]` element facts and record one-push-per-item origin for a separate target array.
