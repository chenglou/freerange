...
4. **Use real demos as pressure tests, then prune.**
   ...
   Corpus loop has started in `/Users/chenglou/github/freerange-corpus` with `tldraw`, `dagre`, `xyflow`, `d3-scale`, and `masonry` on isolated probe branches. The first useful target was `xyflow/packages/system/src/utils/general.ts`; it found general holes, not demo-specific atoms: named `const` arrow helpers, destructured params, silent helper summaries for stored locals, and interval multiplication over unbounded non-negative values. The second useful target was `xyflow` edge geometry; it earned array binding patterns for tuple-style geometry returns, tuple-slot helper summaries, fall-through branch joins, and conservative scalar/object assignment forgetting.
