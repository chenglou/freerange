...
- Import support is deliberately tiny: named/default/namespace-qualified helper imports can use TypeScript resolution to local source, including relative paths, `tsconfig` path aliases, single-source declaration maps back to local source, exported numeric constants for named imports, explicit named re-exports, source `export *` barrels, and top-level `const` helper bindings whose final target is already in the supported call set. Type-field comments also work through local source-backed type imports. Published packages, declaration-only imports without local source maps, summary files, and stale-summary policy are still out.
