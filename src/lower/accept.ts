...
export function assertAccepted(root: ts.Node, checker: ts.TypeChecker): void {
  const visit = (node: ts.Node): void => {
    ...
    // checker types as `any` when asked out of value position.
    ...
    // expects a number. The non-null assertion `x!` is not rejected here; lowering accepts
    // it while it does not change the value kind.
