...
export function localizeContainerLiteralValue(value: Value, expr: string, options: LocalizeOptions = {}): Value {
  ...
  if (value.kind === 'object') {
    ...
    for (const [name, prop] of value.props) {
      props.set(name, prop.kind === 'object' || prop.kind === 'array'
        ? localizeContainerLiteralValue(prop, `${expr}.${name}`, options)
        : prop)
    }
    return {...value, props, expr}
  }
