...
export function numberCaseLossMessage(loss: NumberCaseLoss) {
  switch (loss.kind) {
    ...
    case 'separate-branches':
      return 'Numeric alternatives came from separate branch constructs and are not assumed to match'
  }
}
...
function foldBinary(
  ...
  return withNaNStatus(
    numberValue(result, result, gridOfNumber(result), expr, linearConstant(result), null, origin),
    operandsExcludeNaN(left, right),
  )
}
...
export function numberValue(
  ...
  if (clean != null && clean.terms.size === 0) {
    const exact = rationalToExactNumber(clean.constant)
    if (exact != null) {
...
export function numberBranches(value: NumberValue): NumberCase[] {
  return value.cases ?? [{value: plainNumber(value), assumptions: [], branches: []}]
}

export function joinNumberValues(left: NumberValue, right: NumberValue): NumberValue {
  const joined = withInheritedNumberCaseLoss(mergePlainNumberValues(left, right), left, right)
...
export function withNumberCases(value: NumberValue, cases: NumberCase[] | null): NumberValue {
  ...
  const plainCases = cases.map(choice => ({
    value: plainNumber(choice.value),
  ...
  if (
    normalized.length === 1
    && sameNumberShape(value, normalized[0]!.value)
    && normalized[0]!.assumptions.length === 0
    && normalized[0]!.branches.length === 0
  ) return value
...
export function withCombinedNumberCaseInfo(
  ...
  return loss == null ? value : withNumberCaseLoss(value, loss)
}
...
export function numberCaseCombinationLoss(
  ...
  return left.caseLoss ?? right.caseLoss ?? null
}

function normalizeNumberCases(cases: NumberCase[]): NumberCase[] {
  if (cases.some(choice => choice.assumptions.length > 0 || choice.branches.length > 0)) return cases
  ...
  for (const item of sorted) {
    ...
    result[result.length - 1] = {
      value: mergeNumberCaseValues(previous.value, item.value),
      assumptions: [],
      branches: [],
    }
  }
...
function sameNumberCaseLoss(left: NumberCaseLoss | undefined, right: NumberCaseLoss | undefined) {
  ...
  switch (left.kind) {
    ...
    case 'separate-branches':
      return right.kind === 'separate-branches'
  }
}
...
export function numberWithBounds(
  ...
  const bounded = withInheritedNumberCaseLoss(
...
function zeroIdentity(other: NumberValue, zero: NumberValue, expr: string | null, origin: string[]): NumberValue | null {
  ...
  return withNaNStatus(
    numberValue(other.min, other.max, other.grid, expr, other.linear, null, origin),
    operandsExcludeNaN(other, zero),
  )
}
...
export function addNumbers(left: NumberValue, right: NumberValue): NumberValue {
  ...
  const noNaN = operandsExcludeNaN(left, right)
...
export function multiplyNumbers(left: NumberValue, right: NumberValue): NumberValue {
  ...
  return withNaNStatus(
    numberValue(products.min, products.max, grid, expr, linear, null, origin),
    operandsExcludeNaN(left, right),
  )
}
...
export function divideNumbers(left: NumberValue, right: NumberValue): Value {
  ...
  return withNaNStatus(
    numberValue(quotients.min, quotients.max, grid, expr, linear ?? opaqueLinear(expr), null, origin),
    operandsExcludeNaN(left, right),
  )
}
...
export function moduloNumbers(left: NumberValue, right: NumberValue): Value {
  ...
  const expr = binaryExpr(left, '%', right)
  const linear = opaqueLinear(expr)
  const origin = mergeOrigin(left, right)
  const folded = foldBinary(left, right, (a, b) => a % b, expr, origin)
  if (folded != null) return folded
