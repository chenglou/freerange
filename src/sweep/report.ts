...
export type SweepReport = {findings: SweepFinding[]; summaryLine: string; soundnessError: boolean; json: unknown}
...
export function sweepReport(options: {run: SweepRun; verdictOf: (site: Site) => StaticVerdict; detailed: DetailedAnalysis; staticFindings: StaticFinding[]; sourceFile: ts.SourceFile; settings: SweepSettings; level: 'warning' | 'error'; reportFile: string}): SweepReport {
  ...
  if (run.kind === 'not-run') {
    ...
    return {findings, soundnessError: false, summaryLine: 'sweep: 0 of 0 functions run; 0 asserts held on at least 1,000 generated inputs; 0 counterexamples; 0 functions not run', json: {file: reportFile, settings: settingsJson, status: {kind: 'not-run', reason: run.reason}, entries: [], sites: [], internalErrors: [], caps: {siteCap: siteCap ? 1 : 0}, timing: {}}}
  }
  ...
  return {findings, summaryLine, soundnessError, json}
}
