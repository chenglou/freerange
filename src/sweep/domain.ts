// Input domains and number value sources for domain@v2 and lattice@v1. Ported from mutation-instrument-spike at bccf0dd
// (mutation-instrument/domain.ts), itself forked from the replay input-range prototype (domain.ts). Changes from the port:
// capUnboundedEnds takes the cap as a parameter (FREERANGE_SWEEP_CAP, M3 arm A2), and DOMAIN_VERSIONS is gone. A domain is what an entry function accepts: its parameter types narrowed by the entry's leading
...
/**
 * Caps, in place, every number end that no leading assert bounded: -cap below and cap above. A declared bound replaces
 * the cap on its side, whether it's narrower or wider than the cap. E.g. with cap 1e6, after `x >= 0` and `x <= 1e8` the
 * domain is [0, 1e8], after only `x >= 0` it's [0, 1e6], and with no leading assert it's [-1e6, 1e6].
 */
export function capUnboundedEnds(domain: Domain, cap: number) {
  switch (domain.kind) {
    case 'number':
      if (domain.min === -Infinity) domain.min = -cap
      if (domain.max === Infinity) domain.max = cap
      break
    case 'choice':
      break
    case 'record':
      for (const field of domain.fields) capUnboundedEnds(field.domain, cap)
      break
    case 'array':
      capUnboundedEnds(domain.element, cap)
      break
    case 'tuple':
      for (const element of domain.elements) capUnboundedEnds(element, cap)
      break
    case 'union':
      for (const member of domain.members) capUnboundedEnds(member, cap)
      break
  }
}
