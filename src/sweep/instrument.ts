...
// index, so a child needs no TypeScript and the site check runs before any child starts.
...
export function instrumentSource(text: string, path: string, file: string, siteOffset: number): {output: string; sites: Site[]} {
  ...
  // inside a function expression inside an assert condition. Refuse rather than drop the tick.
