...
// Ported from mutation-instrument-spike at bccf0dd (mutation-instrument/lattice.ts). Changes from the port: the
// domain@v3-callers offset relations are gone (caller rules are not ported), and F2 array-length ties: two array parameter
// paths tied by a leading `A.length === B.length` share one length leaf, so both arrays get the same length.
...
import type {LatticeSettings, Path, SweepEntry} from './types.ts'
...
export type Lattice = {
  ...
  drawnLengths: Int32Array // scratch: a tied array leaf's drawn length for the input being generated, -1 before the draw
}

// F2: the leaf of an already-compiled array this path is tied to, keyed by JSON.stringify(path).
type Ties = {pairs: [Path, Path][]; leaves: Map<string, number>}

function tiedLeaf(ties: Ties, path: Path): number | null {
  const key = JSON.stringify(path)
  for (const [left, right] of ties.pairs) {
    const other = JSON.stringify(left) === key ? JSON.stringify(right) : JSON.stringify(right) === key ? JSON.stringify(left) : null
    if (other == null) continue
    const leaf = ties.leaves.get(other)
    if (leaf != null) return leaf
  }
  return null
}

function compile(domain: Domain, counter: {leaf: number}, inArray: boolean, path: Path | null, ties: Ties): GenNode {
  switch (domain.kind) {
    ...
    case 'array': {
      const tied = inArray || path == null ? null : tiedLeaf(ties, path)
      const leaf = inArray ? -1 : tied ?? counter.leaf++
      if (!inArray && path != null) ties.leaves.set(JSON.stringify(path), leaf)
      return {kind: 'array', leaf, element: compile(domain.element, {leaf: 0}, true, null, ties), maxLength: domain.maxLength}
    }
    case 'record': return {kind: 'record', fields: domain.fields.map((field) => ({name: field.name, node: compile(field.domain, counter, inArray, path == null ? null : [...path, field.name], ties)}))}
    case 'tuple': return {kind: 'tuple', elements: domain.elements.map((element, index) => compile(element, counter, inArray, path == null ? null : [...path, index], ties))}
    case 'union': {
      ...
      for (const member of domain.members) {
        ...
        // A tie never reaches into a union member: which member is drawn decides whether the array exists.
        members.push(compile(member, counter, inArray, null, ties))
...
export function compileLattice(entry: SweepEntry, settings: LatticeSettings): Lattice {
  const counter = {leaf: 0}
  const root = compile(entry.args, counter, false, [], {pairs: entry.lengthTies, leaves: new Map()})
  ...
  return {ordinal: entry.ordinal, settings, root, leafCounts, relations, phases: {p0, p1, p2}, fixed: new Int32Array(leafCounts.length), drawnLengths: new Int32Array(leafCounts.length)}
}

function generate(random: Random, node: GenNode, fixed: Int32Array, drawnLengths: Int32Array, small: boolean): Value {
  switch (node.kind) {
    case 'number': {
    ...
    case 'array': {
      const candidate = node.leaf >= 0 ? fixed[node.leaf]! : -1
      const drawn = node.leaf >= 0 ? drawnLengths[node.leaf]! : -1
      const length = candidate >= 0 ? candidate : drawn >= 0 ? drawn : nextIndex(random, small ? Math.min(3, node.maxLength + 1) : node.maxLength + 1)
      if (node.leaf >= 0) drawnLengths[node.leaf] = length
      const result: Value[] = []
      for (let index = 0; index < length; index++) result.push(generate(random, node.element, fixed, drawnLengths, small))
      return result
    }
    case 'record': {
      const result: Record<string, Value> = {}
      for (const field of node.fields) result[field.name] = generate(random, field.node, fixed, drawnLengths, small)
      return result
    }
    case 'tuple': {
      const result: Value[] = []
      for (const element of node.elements) result.push(generate(random, element, fixed, drawnLengths, small))
      return result
    }
    case 'union': {
      ...
      return generate(random, node.members[chosen]!, fixed, drawnLengths, small)
    }
  }
}
...
export function inputAt(lattice: Lattice, index: number): Input {
  ...
  const args = generate(random, lattice.root, fixed, lattice.drawnLengths, producer === 0) as Value[]
