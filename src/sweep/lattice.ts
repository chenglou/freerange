// lattice@v1: the input sequence of one entry, addressable by index, in phases with a small-scope phase in front:
...
// F2 array-length ties: two array parameter paths tied by a leading `A.length === B.length` share one length leaf, so both arrays get the same length.
