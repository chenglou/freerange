...
describe('control flow and contracts', () => {
  ...
  test('reuses repeated complete calls without conflating their inputs or module state', () => {
    const repeatedCalls = ['function step0(value: number): number { return value + 1 }']
    // The source has 21 distinct call inputs but expands to more than a million runtime
    // calls. Analysis should follow the distinct inputs rather than the runtime call tree.
    for (let depth = 1; depth <= 20; depth++) {
      repeatedCalls.push(
        `function step${depth}(value: number): number { return step${depth - 1}(value) + step${depth - 1}(value) }`,
      )
    }
    const report = analyzeSource('repeated-calls.ts', `
      ...
      const initialized = next() + next()
      export function repeatedPure(value: number): number {
        return step20(value)
      }
      export function initializedTotal(): number {
        return initialized
      }
      export function distinctRequirements(left: number, right: number): number {
        return reciprocal(left) + reciprocal(right)
      }
      export function distinctBounds(left: number[], right: number[], index: number): number {
        return at(left, index) + at(right, index)
      }
    `)

    expect(analyzedFunction(report, 'repeatedPure').kind).toBe('analyzed')
    expect(analyzedFunction(report, 'initializedTotal').ensures)
      .toEqual(['return is a finite integer number from 3 through 3'])
    const requirements = analyzedFunction(report, 'distinctRequirements').requires
    expect(requirements).toHaveLength(2)
    expect(requirements[0]).toContain('left is nonzero')
    expect(requirements[1]).toContain('right is nonzero')
    const bounds = analyzedFunction(report, 'distinctBounds').requires
    expect(bounds).toHaveLength(2)
    expect(bounds[0]).toContain('index is a valid left index')
    expect(bounds[1]).toContain('index is a valid right index')
  })

  test('cached calls preserve the operation that introduced a non-finite value', () => {
    const report = analyzeSource('call-provenance.ts', `
      function keep(value: number): number {
        return value
      }
      export function provenance(leftInput: number, rightInput: number): number {
        const left = leftInput * 2
        const right = rightInput * 3
        keep(left)
        return keep(right)
      }
    `)

    const ensures = analyzedFunction(report, 'provenance').ensures
    expect(ensures).toHaveLength(1)
    expect(ensures[0]).toContain('call-provenance.ts:7:23')
  })
