...
describe('static relations', () => {
  ...
  test('value numbering proves a guard on a recomputed formula and a literal written twice', () => {
    const source = `
      export function recomputedGuard(panelWidth: number) {
        ...
        console.assert(panelWidth <= 3000)
        const labelWidth = Math.max(40, Math.min(320, panelWidth - 24))
        const room = panelWidth - 24
        if (room >= 320) console.assert(labelWidth >= 320)
      }
  ...
  test('sign through order proves a halved slack and a quotient by at least 1, and only for nonnegative bases', () => {
    const source = `
      ...
      export function quotientAtMostBase(width: number, reserved: number, parts: number) {
        ...
        console.assert(parts <= 1000)
        const available = width - reserved
        if (available >= 0) {
          const share = available / parts
          console.assert(share <= available)
          console.assert(share <= width)
        }
      }
