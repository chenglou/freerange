...
describe('static relations', () => {
  ...
  test('value numbering proves a guard on a recomputed formula and a literal written twice', () => {
    const source = `
      export function recomputedGuard(panelWidth: number) {
        console.assert(panelWidth >= 0)
        console.assert(panelWidth <= 5000)
        const labelWidth = Math.max(32, Math.min(400, panelWidth - 24))
        const roomLeft = panelWidth - 24
        if (roomLeft >= 400) console.assert(labelWidth >= 400)
      }
      export function literalFactorTwice(weeks: number) {
        console.assert(Number.isInteger(weeks))
        console.assert(weeks >= 0)
        console.assert(weeks <= 1000)
        const cappedDays = Math.min(weeks, 52) * 7
        const allDays = weeks * 7
        console.assert(allDays >= cappedDays)
      }
    `
  ...
  test('order closure chains recorded facts, tracks strictness, and answers max(xs) <= min(ys)', () => {
    const source = `
      ...
      export function excerptSpan(textLength: number, from: number, to: number, glyphWidth: number) {
        console.assert(textLength >= 0)
        console.assert(textLength <= 80000)
        console.assert(from >= 0)
        console.assert(from <= 80000)
        console.assert(to >= 0)
        console.assert(to <= 80000)
        console.assert(glyphWidth >= 0)
        console.assert(glyphWidth <= 64)
        const length = Math.floor(textLength)
        const scale = Math.max(1, Math.floor(glyphWidth))
        const high = Math.min(length, Math.max(Math.floor(from), Math.floor(to)))
        const low = Math.max(0, Math.min(Math.min(Math.floor(from), Math.floor(to)), high))
        const lowOffset = Math.min(length * 64, low * scale)
        const highOffset = Math.min(length * 64, high * scale)
        console.assert(low <= high)
        console.assert(lowOffset <= highOffset)
      }
    `
    ...
    expect(verdictsOffAndOn(source, 'excerptSpan')).toEqual({off: ['unproven', 'unproven'], on: ['proven', 'proven']})
  })
  ...
  test('sign through order proves a halved slack and a quotient by at least 1, and only for nonnegative bases', () => {
    const source = `
      ...
      export function quotientAtMostBase(budget: number, spent: number, shares: number) {
        console.assert(budget >= 0)
        console.assert(budget <= 2000000)
        console.assert(spent >= 0)
        console.assert(spent <= 2000000)
        console.assert(Number.isInteger(shares))
        console.assert(shares >= 1)
        console.assert(shares <= 500)
        const remaining = budget - spent
        if (remaining >= 0) {
          const perShare = remaining / shares
          console.assert(perShare <= remaining)
          console.assert(perShare <= budget)
        }
      }
  ...
  test('float round trips and cancellation stay unproven', () => {
    ...
    // The guard written as price + fee <= budget instead of price <= budget - fee: on doubles
    // price can exceed fl(budget - fee) by rounding while the sum still fits, so paid === price
    // is false there, while the spelling that reuses budget - fee proves.
    const payable = (name: string, guard: string): string => `
      export function ${name}(price: number, fee: number, budget: number) {
        console.assert(fee >= 0)
        console.assert(fee <= 6000)
        console.assert(budget >= 0)
        console.assert(budget <= 6000)
        console.assert(price >= 0)
        console.assert(price <= 6000)
        const paid = Math.min(price, Math.max(0, budget - fee))
        const leftover = budget - fee
        const total = price + fee
        if (${guard}) {
          console.assert(paid === price)
        }
      }
    `
    expect(verdictsOffAndOn(payable('inverted', 'total <= budget'), 'inverted').on).toEqual(['unproven'])
    expect(verdictsOffAndOn(payable('reused', 'price <= leftover'), 'reused').on).toEqual(['proven'])
  })
  ...
  test('join facts hold on every arm of a clamp, keep a sign, and drop when an arm breaks the bound', () => {
    const source = `
      ...
      export function restockOrHold(urgent: boolean, capacity: number, requested: number, held: number, reserve: number): number {
        console.assert(capacity >= 0)
        console.assert(capacity <= 900)
        console.assert(requested >= 0)
        console.assert(requested <= 900)
        console.assert(held >= 0)
        console.assert(held <= 900)
        console.assert(reserve >= 0)
        console.assert(reserve <= 900)
        const shipped = Math.min(requested, capacity)
        const remaining = urgent ? capacity - shipped : held
        const stocked = reserve + remaining
        console.assert(reserve <= stocked)
        return stocked
      }
    ...
    expect(verdictsOffAndOn(source, 'restockOrHold')).toEqual({off: ['unproven'], on: ['proven']})
  ...
  test('integer offsets move a bound across an exact addition, and never past 2^53', () => {
    const offset = (name: string, limit: string): string => `
      export function ${name}(itemCount: number, cursor: number) {
        console.assert(Number.isInteger(itemCount))
        console.assert(itemCount >= 0)
        console.assert(itemCount <= ${limit})
        console.assert(Number.isInteger(cursor))
        console.assert(cursor >= -10)
        console.assert(cursor <= ${limit})
        const lastTaken = Math.min(itemCount - 1, cursor)
        const taken = lastTaken + 1
        console.assert(taken <= itemCount)
        console.assert(lastTaken < taken)
      }
    `
    expect(verdictsOffAndOn(offset('safeOffset', '10000'), 'safeOffset')).toEqual({off: ['unproven', 'unproven'], on: ['proven', 'proven']})
    // itemCount = 2^53 + 2: itemCount - 1 rounds to 2^53, and 2^53 + 1 rounds back to 2^53, so
    // lastTaken < taken is false.
  ...
  test('integer linear forms prove offsets over uncomputed differences, and never for floats, past 2^53, or with one operand of a selection', () => {
    const source = `
      export function tokenBudget(limit: number, turns: number) {
        console.assert(Number.isInteger(limit))
        console.assert(limit >= 1)
        console.assert(limit <= 20000)
        console.assert(Number.isInteger(turns))
        console.assert(turns >= 0)
        console.assert(turns <= 5000)
        const reserved = Math.min(limit, 3 * (turns + 4))
        const spent = Math.min(limit - 2, turns * 3 + 9)
        const usable = reserved - 1
        console.assert(spent <= usable)
        console.assert(spent < reserved)
      }
      ...
      export function tokenOffByOne(limit: number, turns: number) {
        console.assert(Number.isInteger(limit))
        console.assert(limit >= 1)
        console.assert(limit <= 20000)
        console.assert(Number.isInteger(turns))
        console.assert(turns >= 0)
        console.assert(turns <= 5000)
        const reserved = Math.min(limit, 3 * turns + 2)
        const spent = Math.min(limit - 1, 3 * turns + 2)
        const usable = reserved - 1
        console.assert(spent <= usable)
      }
    ...
    `
    expect(verdictsOffAndOn(source, 'tokenBudget')).toEqual({off: ['unproven', 'unproven'], on: ['proven', 'proven']})
    ...
    // turns = 0, limit = 5: spent = 2 and usable = 1.
    expect(verdictsOffAndOn(source, 'tokenOffByOne').on).toEqual(['unproven'])
  ...
  test('linear form caps: a ninth leaf keeps the sum as one leaf, and the substitution search stops at its depth', () => {
    ...
    // Proving chain <= (k + 1) * 3 + 2 walks down every Math.min to 3 * k + 5, one substitution
    ...
    const chainSource = (name: string, levels: number): string => {
      ...
      return `
        export function ${name}(k: number, ${others.map(other => `${other}: number`).join(', ')}) {
          ...
          const m0 = Math.min(3 * k + 5, other0)
  ...
  test('extension budget: an expensive integer search runs out on its own budget, and the first prototype keeps its proofs', () => {
    ...
    // v10 < v0, as they did before the second exploration's rules existed. (The first prototype also
    // refuted v6 === b through a relational rule; relational rules no longer refute, see the next test.)
    const source = `
  ...
  test('arm splitting proves every incoming argument against a bound computed after the join, and stops at the depth cap', () => {
    const source = `
      export function receiptTotal(items: number, unitPrice: number, express: boolean) {
        console.assert(Number.isInteger(items))
        console.assert(items >= 0)
        console.assert(items <= 20000)
        console.assert(unitPrice >= 1)
        console.assert(unitPrice <= 800)
        const billable = Math.min(items, express ? 2 : 5)
        const total = billable === 0 ? 0 : billable * unitPrice + 4
        const fullOrder = 5 * unitPrice + 4
        const limit = fullOrder + 0.001
    ...
    `
    expect(verdictsOffAndOn(source, 'receiptTotal')).toEqual({off: ['unproven'], on: ['proven']})
  ...
  test('return relations publish the order between a call result and its arguments that holds on every return path', () => {
    const source = `
      function raiseTo(value: number, floor: number, ceiling: number): number {
        return Math.min(Math.max(value, floor), Math.max(floor, ceiling))
      }
      export function granted(requested: number, bonus: boolean, cap: number) {
        console.assert(requested >= -1000)
        console.assert(requested <= 1000)
        console.assert(cap >= 0)
        console.assert(cap <= 1000)
        const minimum = bonus ? 20 : 10
        const amount = raiseTo(requested, minimum, cap)
        console.assert(amount >= minimum)
      }
    ...
    `
    expect(verdictsOffAndOn(source, 'granted')).toEqual({off: ['unproven'], on: ['proven']})
    ...
    const off = requirementsBesidesInputFiniteness(analyzedFunction(analyze(source, false), 'granted'))
    const on = requirementsBesidesInputFiniteness(analyzedFunction(analyze(source, true), 'granted'))
    expect(on).toEqual(off)
  })
