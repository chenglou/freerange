...
function measure(prev: Layout): Layout {
  ...
  // Bug: the cache key keeps the old font size, so every later frame re-measures.
  const next = {fontSize: prev.fontSize, width: prev.width, measuredFontSize: prev.measuredFontSize, measuredWidth: prev.width, columns, version: prev.version + 1}
  return next
}
