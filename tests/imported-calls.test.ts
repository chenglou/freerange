...
const feed = `export function feedVertical(inputBottom: number, gap: number, viewportWidth: number) {
  console.assert(gap >= 0)
  const subnavTop = inputBottom + gap
  const columnCount = Math.max(1, Math.floor(viewportWidth / 320))
  const layout = {subnavTop, columnCount}
  console.assert(inputBottom <= layout.subnavTop)
  return layout
}
`

const page = (feedModule: string): string => `import {feedVertical} from './${feedModule}'

export function pageFrame(windowGapTop: number, inputBarHeight: number, viewportWidth: number) {
  const top = Math.min(Math.max(0, windowGapTop), 200)
  const inputBottom = top + Math.min(Math.max(0, inputBarHeight), 400)
  const feed = feedVertical(inputBottom, 4, viewportWidth)
  const frame = {
    input: {top, bottom: inputBottom},
    content: {top: feed.subnavTop, columnCount: feed.columnCount},
  }
  console.assert(frame.input.bottom <= frame.content.top)
  console.assert(frame.content.columnCount >= 1)
  return frame
}
`
...
const mainProject = memoized(() => writeProject({
  ...
  'feed.ts': feed,
  'page.ts': page('feed'),
  'feed-range.ts': replaceOnce(feed, 'Math.max(1, ', 'Math.max(0, '),
  'page-range.ts': page('feed-range'),
  'feed-relation.ts': replaceOnce(feed, 'inputBottom + gap', 'inputBottom + gap - 4'),
  'page-relation.ts': page('feed-relation'),
  'feed-requirement.ts': replaceOnce(feed, 'console.assert(gap >= 0)', 'console.assert(gap >= 8)'),
  'page-requirement.ts': page('feed-requirement'),
  ...
  'guarded-feed.ts': `export function guardedFeed(inputBottom: number, gap: number) {
  const subnavTop = inputBottom + gap
  if (gap >= 0) {
    console.assert(inputBottom <= subnavTop)
  }
  return {subnavTop}
}
`,
  'guarded-page.ts': `import {guardedFeed} from './guarded-feed'

export function guardedCaller(inputBottom: number, gap: number) {
  const layout = guardedFeed(inputBottom, gap)
  console.assert(inputBottom <= layout.subnavTop)
  return layout
}
`,
  ...
  'two-returns-feed.ts': `export function twoReturnsFeed(inputBottom: number, gap: number) {
  console.assert(gap >= 0)
  const subnavTop = inputBottom + gap
  console.assert(inputBottom <= subnavTop)
  if (gap > 100) return {subnavTop: inputBottom - 1}
  return {subnavTop}
}
`,
  'two-returns-page.ts': `import {twoReturnsFeed} from './two-returns-feed'

export function twoReturnsCaller(inputBottom: number) {
  const layout = twoReturnsFeed(inputBottom, 200)
  console.assert(inputBottom <= layout.subnavTop)
  return layout
}
`,
...
export const baseWidth = measure(8)
...
test('with imported calls off, a call into another file keeps rejecting the caller', () => {
  expect(findingLines(offFindings())).toContain('page.ts(6,16): error [console-assert]: console.assert in pageFrame was not checked because function call feedVertical')
})
...
describe('contract: what a caller may rely on', () => {
  test("module B's checks catch each edit to module A", () => {
    ...
    expect(findings.filter(line => line.startsWith('page.ts(') || line.startsWith('feed.ts('))).toEqual([])
    // A range edit: A's column count may now be 0.
    expect(findings).toContain('page-range.ts(12,3): error [console-assert]: could not prove console.assert condition in pageFrame: frame.content.columnCount >= 1')
    // A relation edit: A's relation is no longer proven, so B loses the fact it relied on.
    expect(findings).toContain('feed-relation.ts(6,3): error [console-assert]: could not prove console.assert condition in feedVertical: inputBottom <= layout.subnavTop')
    expect(findings).toContain('page-relation.ts(11,3): error [console-assert]: could not prove console.assert condition in pageFrame: frame.input.bottom <= frame.content.top')
    // A stricter requirement: B passes a gap of 4, and the related location names A's file.
    expect(findings).toContain('page-requirement.ts(6,16): error [declared-requirement]: call to feedVertical makes its declared requirement definitely false (declared at feed-requirement.ts(2,3))')
  })

  test('a relation publishes only from a proven assertion that dominates the only return', () => {
    expect(functionBlock(contractAudit(), 'guarded-page.ts', 'guardedCaller')).toContain('assertion unproven: could not prove inputBottom <= layout.subnavTop (at guarded-page.ts:5:3)')
    expect(functionBlock(contractAudit(), 'two-returns-feed.ts', 'twoReturnsFeed')).toContain('proves: inputBottom <= subnavTop (assertion at two-returns-feed.ts:4:3)')
    expect(functionBlock(contractAudit(), 'two-returns-page.ts', 'twoReturnsCaller')).toContain('assertion unproven: could not prove inputBottom <= layout.subnavTop (at two-returns-page.ts:5:3)')
  })
...
`,
  ...
  test(`a path into a returned record is searched through at most ${maximumReturnPathSteps} values`, () => {
    ...
    const layout = (layoutName: string, fields: number): string => {
      ...
      return `export function ${layoutName}(inputBottom: number, gap: number) {
  console.assert(gap >= 0)
  const subnavTop = inputBottom + gap
  console.assert(inputBottom <= subnavTop)
  return {
...
export function wideCaller(inputBottom: number) {
  const layout = wideLayout(inputBottom, 4)
  console.assert(inputBottom <= layout.subnavTop)
  return layout.subnavTop
}

export function narrowCaller(inputBottom: number) {
  const layout = narrowLayout(inputBottom, 4)
  console.assert(inputBottom <= layout.subnavTop)
  return layout.subnavTop
}
`,
    ...
    expect(functionBlock(audit, 'layout-user.ts', 'wideCaller')).toContain('assertion unproven: could not prove inputBottom <= layout.subnavTop (at layout-user.ts:6:3)')
    expect(functionBlock(audit, 'layout-user.ts', 'narrowCaller')).toContain('proves: inputBottom <= layout.subnavTop (assertion at layout-user.ts:12:3)')
  })
})
