...
describe('static console.assert contracts', () => {
  ...
  test('an unsupported interior condition leaves only its own assertion unchecked', () => {
    const report = analyzeSource('assertion-not-checked.ts', `
      ...
      export function toolbarFrame(windowWidth: number, rawLabelWidth: number): {label: number; toolbarLeft: number} {
        const width = Math.max(400, windowWidth)
        const label = Math.max(0, Math.min(rawLabelWidth, 100))
        const toolbarLeft = width - 50
        console.assert(label >= 0)
        console.assert(label <= toolbarLeft - 16)
        console.assert(label === 0 || label > 0)
        console.assert(label <= 100)
        return {label, toolbarLeft}
      }
      export function doubledToolbarLeft(windowWidth: number): number {
        return toolbarFrame(windowWidth, 80).toolbarLeft * 2
      }
      let scale = 1
    ...
    const toolbarFrame = analyzedFunction(report, 'toolbarFrame')
    expect(verdicts(toolbarFrame)).toEqual(['proven', 'notChecked', 'notChecked', 'proven'])
    expect(toolbarFrame.assertions?.[1]).toMatchObject({
      verdict: 'notChecked',
      text: 'label <= toolbarLeft - 16',
      reason: 'calculate or read the value before console.assert, then check the variable',
    })
    expect(toolbarFrame.ensures).toEqual([
      'return.label is a finite number from 0 through 100',
      'return.toolbarLeft is a finite number at least 350',
    ])
    expect(analyzedFunction(report, 'doubledToolbarLeft').ensures[0])
